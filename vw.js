...
async function main() {
    ...
    if (code == '000000') {
        ...
        pushMsg(`获取临时Token失败，code: ${code || '未知'}，请检查 token 是否正确或重新获取 CK`);
    }
}
...
function csvw(obj) {
    return new (class {
        ...
        getid() {
            // 解析 refreshToken 里的用户信息（JWT 中间那段）
            if (this.refreshToken && this.refreshToken.split('.').length >= 2) {
                try {
                    const payloadBase64 = this.refreshToken.split('.')[1];
                    const jsonStr = CryptoJS.enc.Utf8.stringify(
                        CryptoJS.enc.Base64.parse(payloadBase64)
                    );
                    const obj = $.toObj(jsonStr) || {};
                    this.openId = obj.ssoid;
                    this.userId = obj.sub;
                } catch (e) {
                    $.log(`解析 refreshToken 失败: ${e}`);
                    throw new Error('解析 refreshToken 出错，请重新抓取 token');
                }
            } else {
                throw new Error('refreshToken 格式不正确，请重新抓取 token');
            }
        }
        ...
        async refresh() {
            ...
            // 使用 _method 指定 POST（原代码 method: 'PUT' 实际没用到）
            const rest = { url, body, headers, _method: 'post' };
            const res = await httpRequest(rest, '获取临时Token ');
            const { code, data, description, raw } = res || {};

            $.log('获取临时Token 返回原始对象: ' + $.toStr(res));
            pushMsg(`获取临时Token:${description || code || '无描述'}`);
        ...
        async refreshSxToken() {
            ...
            pushMsg(`刷新userToken:${obj?.description || obj?.message || '无描述'}`);
        }
        ...
        async sign() {
            ...
            let _msg;
            if (obj?.code == '000000') {
                _msg = `签到:成功，已签到${obj?.data?.signCount}天`;
            } else {
                _msg = `❌签到失败:${obj?.description || obj?.message || obj?.code || '未知错误'}!`;
            }
            pushMsg(_msg);
        }
        ...
        async status() {
            ...
            let _msg;
            if (obj?.code == '000000') {
                _msg = `当前积分:${obj?.data?.pointCount}`;
            } else {
                _msg = `❌查积分失败:${obj?.description || obj?.message || obj?.code || '未知错误'}`;
            }
            pushMsg(_msg);
        }
...
function getCk() {
    if ($request && $request.method != 'OPTIONS') {
        ...
        const did = obj.did;
        const token = (obj.authorization || '').replace(/^Bearer\s*/i, '');
...
!(async () => {
    typeof $request !== `undefined`
        ? await getCk()
        : (intbase64(), await main());  // 主函数
})().catch((e) => {
    $.messages.push(e && e.message ? e.message : $.toStr(e));
    $.logErr(e);
}).finally(async () => {
...
async function httpRequest(options, 调试备注) {
    ...
    try {
        options = options && options.url ? options : { url: options };
        const _method   = (options._method || (('body' in options) ? 'post' : 'get')).toLowerCase();
        ...
        return await Promise.race([
            ...
            new Promise((_, reject) =>
                setTimeout(() => reject(new Error(`⛔️ 请求超时: ${options.url}`)), _timeout)
            ),
            ...
            new Promise((resolve, reject) => {
                ...
                if (typeof $[_method] !== 'function') {
                    return reject(new Error(`不支持的请求方法: ${_method}`));
                }

                $[_method](options, (error, response, data) => {
                    debug(data, 调试备注 + '[响应body]');

                    if (error) {
                        $.log(`HTTP 请求失败: ${$.toStr(error)}`);
                        return reject(typeof error === 'string' ? new Error(error) : error);
                    }

                    if (!response) {
                        return reject(new Error('HTTP 响应为空（response 为 undefined）'));
                    }
                    ...
                    const raw = response[_respType];
                    const parsed = $.toObj(raw, null);

                    if (parsed && typeof parsed === 'object') {
                        return resolve(parsed);
                    } else {
                        // 非 JSON 的情况也返回一个正常对象，避免上层解构炸掉
                        return resolve({
                            code: 'PARSE_ERROR',
                            description: '返回不是 JSON，查看 raw 字段',
                            raw
                        });
                    }
                });
...
function getEnv(...keys){
    for(let key of keys){
        var value = $.isNode()
            ? process.env[key] || process.env[key.toUpperCase()] || process.env[key.toLowerCase()] || $.getdata(key)
            : $.getdata(key);
