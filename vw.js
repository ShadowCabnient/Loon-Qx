...
async function main() {
    const csv = new csvw($.huihui);
    const code = await csv.refresh();
    if (code === '000000') {
...
function csvw(obj) {
    return new (class {
        ...
        async refreshSxToken() {
            ...
            pushMsg(`刷新userToken:${obj?.description || obj?.message || obj?.msg || ''}`);
        }
        ...
        async sign() {
            ...
            let _msg;
            _msg = obj?.code === '000000'
                ? `签到:成功，已签到${obj?.data?.signCount}天`
                : `❌签到失败:${obj?.description || obj?.message || obj?.msg || obj?.code || '未知错误'}!`;
            pushMsg(_msg);
        }
        ...
        async status() {
            ...
            if (obj?.code === '000000') {
                ...
                _msg = `❌查积分失败:${obj?.description || obj?.message || obj?.msg || obj?.code || '未知错误'}`;
            }
            pushMsg(_msg);
        }
...
!(async () => {
    typeof $request !== `undefined`
        ? await getCk()
        : (intbase64(), await main());
})().catch((e) => {
    $.messages.push(e && e.message ? e.message : e);
    $.logErr(e);
}).finally(async () => {
    ...
    try {
        ...
        const httpPromise = new Promise((resolve, reject) => {
            ...
            $[_method](options, (error, response, data) => {
                ...
                if (!response) {
                    return reject(new Error('HTTP 响应为空'));
                }
