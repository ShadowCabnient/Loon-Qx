            ...
            if (this.refreshToken) {
                const obj = $.toObj(
                    CryptoJS.enc.Utf8.stringify(
                        CryptoJS.enc.Base64.parse(this.refreshToken.split('.')[1])
                    )
                );
...
async function httpRequest(options, 调试备注) {
    ...
    try {
        ...
        const _http = [
            ...
            new Promise((resolve, reject) => {
                ...
                $[_method.toLowerCase()](options, (error, response, data) => {
                    ...
                    // 原来只 log 不 reject，这里改为真正抛错
                    if (error) {
                        $.log($.toStr(error));
                        return reject(error);
                    }

                    // 没有 response 也视为错误，避免上层解构 undefined
                    if (!response) {
        ...
        $.logErr(err);
        // 这里抛出去，让上层 catch，而不是悄悄返回 undefined
        throw err;
    }
}
